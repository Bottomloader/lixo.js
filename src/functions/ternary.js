module.exports = {
    ...
    execute: async (context, args) => {
        ...
        const elseValue = args[2] !== undefined ? args[2] : "";
        ...
        try {
            ...
            if (conditionParts) {
                ...
                try {
                    val1 = await context.client.functionParser.parse(val1, context);
                } catch (parseError) {
                    console.warn(`$ternary: Error parsing val1 "${conditionParts[1].trim()}": ${parseError.message}`);
                }

                try {
                    val2 = await context.client.functionParser.parse(val2, context);
                } catch (parseError) {
                    console.warn(`$ternary: Error parsing val2 "${conditionParts[3].trim()}": ${parseError.message}`);
                }
                ...
                switch (op) {
                    case '==':
                        conditionResult = areNumbers ? num1 === num2 : String(val1) === String(val2);
                        break;
                    case '!=':
                        conditionResult = areNumbers ? num1 !== num2 : String(val1) !== String(val2);
                        break;
                    case '>':
                        conditionResult = areNumbers ? num1 > num2 : false;
                        break;
                    case '<':
                        conditionResult = areNumbers ? num1 < num2 : false;
                        break;
